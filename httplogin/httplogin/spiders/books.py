...
# class BooksSpider(scrapy.Spider):
    ...
    # def start_requests(self):
        # url = 'http://kindlepush.com/user/login'

        # # FormRequest 是Scrapy发送POST请求的方法
        # yield scrapy.FormRequest(
            # url = url,
            # formdata = {'username': '1432352091@qq.com', 'password' : 'wxj7153617266', 'rememberMe': 'false'},
            # callback = self.parse
        # )
    # def parse(self, response):
         # yield scrapy.Request('http://kindlepush.com/category',callback=self.parse_book)

    # def parse_book(self, response):
        # for href in response.xpath('//li[@class="u-bookitm1 j-bookitm"]/div[@class="info"]/div/a/@href'):
            # yield response.follow(href, self.parse_name)

    # def parse_name(self, response):
        # yield scrapy.FormRequest(
            # url = 'http://kindlepush.com/upload/download/',
            # formdata = {'resourceId': '37757'},
            # callback = self.parse_a
        # )
    # def parse_a(self, response):
    
        # for book in response.xpath('//div[@class="desc"]'):
            # yield {
                # 'file_name': book.xpath('.//h3/div/text()').extract_first(),
            # }
